...
export default function SavedCities({ cities, onSelect, onRemove, active }) {
  ...
  const Row = memo(function Row({ index, style, items, onSelect, onRemove, active }) {
    const c = items[index];
    const isActive =
      !!active && Number(c.lat) === Number(active.lat) && Number(c.lon) === Number(active.lon);

    return (
      <div style={{ ...style, padding: '4px 0' }}>
        <div className={isActive ? 'saved-item active' : 'saved-item'}>
          <button className="saved-name" onClick={() => onSelect(c)} title={c.name}>
            {c.name}
          </button>
          <button className="icon-btn" title="Remove" onClick={() => onRemove(c)}>
            ✕
          </button>
        </div>
      </div>
